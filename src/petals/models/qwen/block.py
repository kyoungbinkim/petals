...
class OptimizedQwenAttention(Qwen3Attention):
...
class OptimizedQwenDecoderLayer(Qwen3DecoderLayer):
    def __init__(self, config: Qwen3Config):
        nn.Module.__init__(self)
        self.hidden_size = config.hidden_size
        self.self_attn = OptimizedQwenAttention(config=config, layer_idx=0)
...
class WrappedQwenBlock(OptimizedQwenDecoderLayer):
    def forward(
        self,
        hidden_states: torch.Tensor,
        *args,
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            ...
            past_key_value = self._reorder_cache_from_bloom_to_qwen(past_key_value, batch_size, past_key_values_length)
        ...
        if use_cache:
            ...
            present_key_value = self._reorder_cache_from_qwen_to_bloom(
    ...
    def _reorder_cache_from_bloom_to_qwen(
    ...
    def _reorder_cache_from_qwen_to_bloom(
        self, key_value: Tuple[torch.Tensor], batch_size: int, seq_length: int
    ) -> Tuple[torch.Tensor]:
        key_states, value_states = key_value
